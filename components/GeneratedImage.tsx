...
const GeneratedImage: React.FC<GeneratedImageProps> = ({ src, index, isSelected, onSelect, isRemixing, onPreview, style }) => {
  ...
  const borderClass = isSelected ? 'ring-4 ring-offset-2 ring-offset-slate-900 ring-cyan-400' : 'ring-2 ring-transparent';

  return (
    <div 
      className={`relative group aspect-square overflow-hidden rounded-lg shadow-lg transition-all duration-300 animate-reveal ${borderClass} ${isRemixing ? 'cursor-not-allowed' : 'cursor-pointer'}`}
      ...
      style={style}
    >
      <img src={src} alt={`Generated image ${index + 1}`} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
